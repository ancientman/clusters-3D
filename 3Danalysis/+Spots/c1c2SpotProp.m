...
for i=1:nNuc
    if ~isempty(c2SpotStruct(i).center) && ~isempty(c1SpotStruct(i).center)
        if length(c2SpotStruct(i).vol)==1
            c1SpotCentUM = [xPixUM*c1SpotStruct(i).center(:,1), yPixUM*c1SpotStruct(i).center(:,2), zPixUM*c1SpotStruct(i).center(:,3)];
            c2SpotCentUM = [xPixUM*c2SpotStruct(i).center(:,1), yPixUM*c2SpotStruct(i).center(:,2), zPixUM*c2SpotStruct(i).center(:,3)];
...
C1C2SpotStruct.C1C2SpotVal = C1C2NucVal;
end