function combine2CSpotPlotter5(folderPath)
cd(folderPath);
files=dir('*.mat');
fileNames = {files.name};
struct2C = cellfun(@(x)load(append(folderPath, filesep, x)), fileNames, 'un', 0);

% distLim =  [0.485 0.546 0.394 0.333 1];
distLim =  [0.39 0.36];
...
% colorStruct{1} =  [35, 110, 196; 0, 0, 0]; % hunchback
...
colorStruct{1} =  [83, 185, 234]; % bottleneck
colorStruct{2} =  [192, 95, 211]; % hunchback
% groupOrder = [3 2 5 4 1]; % default
%------------------------------------------------------------------%
...
%     geneName(i) = geneNameStruct{j};
    ...
    c1VolMean{i} = cell(1, length(c1Val{i}));
    ...
    c1MolMean{i} = c1ValMean{i}.*c1VolMean{i};
    ...
    coupledMeanC1Mol{i} = c1MolMean{i}(c1DistClose1{i}<=distLim(i) & nucVal{i}>nucValCutOff(1) & nucVal{i}<=nucValCutOff(2));
...
hold on;
h2 = histogram(data,'Normalization','pdf', 'DisplayStyle', 'stairs', EdgeColor=colorTemp);
hold on;
h1.FaceColor = colorTemp;
h1.FaceAlpha = 0;
h1.EdgeColor = colorTemp;
h1.EdgeAlpha = 0.0;
...
axLabel = "NN cluster distance ({\mu}m)";
...
% 
figure('Color', 'w');
fitScatter2(vertcat(nucValRepAllTrim{:}), vertcat(c1ValAllTrim{:}).*vertcat(c1VolAllTrim{:}), [0.8 0.8 0.8]);
hold on;
for i = 1:length(struct2C)
...
[f,Ib] = sort(f);
scatter((X(Ib)),Y(Ib),10,color(1,:),'filled')
hold on;
